...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="converged_only",
        choices=["converged_or_cap", "converged_only"],
        help=(
            "converged_or_cap: promote when converged or max-chunk cap reached. "
            "converged_only: promote only when convergence gates are met."
        ),
    )
    p.add_argument(
...
def _write_merged_rollouts_csv(path: Path, rows: List[Dict]):
    path.parent.mkdir(parents=True, exist_ok=True)
    if len(rows) == 0:
...
def main():
    ...
    if args.curriculum_mode == "adaptive":
        print(
            ...
            f" policy={args.adaptive_promotion_policy},"
        ...
        if args.adaptive_promotion_policy == "converged_only" and int(args.adaptive_max_chunks_per_level) > 0:
            print(
                "[INFO] adaptive max chunk cap is ignored in converged_only mode.",
                flush=True,
            )
    ...
    for i in range(num_chunks):
        ...
        for row in rollouts:
            ...
            merged["chunk_level"] = int(preset.level)
            merged["chunk_name"] = str(preset.name)
        ...
        if args.curriculum_mode == "adaptive":
            ...
            if adaptive_stage_idx < (len(phase_levels) - 1):
                ...
                force_promote = False
                if args.adaptive_promotion_policy == "converged_or_cap":
                    force_promote = bool(
